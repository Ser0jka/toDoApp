...
def create_task(payload: TaskCreateShema, task_service: TaskService = Depends(get_task_service)) -> TaskShema:
    return task_service.create_task(task_create=payload)

@router.patch("/{task_id}")
def update_task(task_id: str, payload: TaskUpdateShema, task_service: TaskService = Depends(get_task_service)) -> TaskShema:
    try:
