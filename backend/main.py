...
def update_category(cat_id: str, payload: CategoryUpdate, db: Session = Depends(get_db)) -> Category:
    category = db.get(CategoryORM, cat_id)
    if category is None:
...
def delete_category(cat_id: str, db: Session = Depends(get_db)) -> None:
    category = db.get(CategoryORM, cat_id)
    if category is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Категория не найдена")
    db.delete(category)
